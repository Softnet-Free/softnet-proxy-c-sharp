...
{
    class UdpConnectorV4 : IUdpConnector, STContext
    {
        ...
        public byte[] HostPrivateIEP = null;

        public readonly Guid EndpointUid;
        public readonly long deathTime;

        public UdpConnectorV4(MsgSocket msgSocket)
        {
        ...
        public void AttachEndpoint(SocketAsyncEventArgs saea, int udpOffset)
        {
            ...
            Buffer.BlockCopy(m_HostIEP.Address.GetAddressBytes(), 0, hostIep, 0, 4);
            Buffer.BlockCopy(saea.Buffer, udpOffset, hostIep, 4, 2);

            byte[] hostPrivateIep = new byte[6];
            Buffer.BlockCopy(saea.Buffer, udpOffset + 25, hostPrivateIep, 0, 6);
            ...
            lock (mutex)
            {
                ...
                if (ByteArray.Equals(hostPrivateIep, 0, hostIep, 0, 4) == false)
                    HostPrivateIEP = hostPrivateIep;
        ...
        public void CreateP2PConnection(byte[] remoteIEP, byte[] remotePrivateIEP, Guid remoteEndpointUid)
        {
            ...
            m_MsgSocket.Send(MsgBuilder.Create(Constants.UdpConnector.CREATE_P2P_CONNECTION_IN_DUAL_MODE, asnEncoder));
        }
