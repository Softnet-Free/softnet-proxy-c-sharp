...
{
    class UdpProxyV4 : IUdpProxy, Monitorable
    {
        long m_EndpointKey;
        ...
        public UdpProxyV4(long endpointKey, IPEndPoint hostIEP, byte[] hostIEPBytes)
        {
            m_EndpointKey = endpointKey;
            m_HostIEP = hostIEP;
            host_port_higher_byte = hostIEPBytes[4];
            host_port_lower_byte = hostIEPBytes[5];
        }
        
        public void Init()
        {
            LastPacketReceiveTime = SystemClock.Seconds;
            m_pseudoHeader = UdpPseudoHeader.GetPHv4(m_HostIEP.Address);
        }
        ...
        public bool IsAlive(long currentSeconds)
        {
            ...
            m_closed = true;
            UdpDispatcher.RemoveProxyV4(this, m_EndpointKey);
            return false;
        }
        ...
        public void HandlePacket(SocketAsyncEventArgs saea, int udpOffset, int udpSize)
        {
            if (m_closed == false)
            {
                LastPacketReceiveTime = SystemClock.Seconds;
                m_PeerProxy.SendPacket(saea, udpOffset, udpSize, LastPacketReceiveTime);
            }
            else
            {
        ...
        public void SendPacket(System.Net.Sockets.SocketAsyncEventArgs saea, int udpOffset, int size, long currentTime)
        {
            if (m_closed == false)
            {
                ...
                UInt16 checksum = Algorithms.ChecksumUdpV4(m_pseudoHeader, buffer, udpOffset, size);
