...
{
    class TcpProxyV4 : ITcpProxy, Monitorable
    {
        long m_EndpointKey;
        ...
        public TcpProxyV4(long endpointKey)
        {
        ...
        public void Remove()
        {
            ...
            TcpDispatcher.RemoveProxyV4(m_EndpointKey);
        }
        ...
        public void HandleSegment(SocketAsyncEventArgs saea, int tcpOffset, int size)
        {
            if (m_ConnectionState == ConnectionState.ESTABLISHED)
            {
                LastSegmentReceivedSeconds = SystemClock.Seconds;
                m_PeerProxy.SendSegment(saea, tcpOffset, size);
            }
            else
            {
                if (m_ConnectionState == ConnectionState.WAITING_SYN)
                {
                    HandleState_WaitingSyn(saea, tcpOffset);
                }
                else if (m_ConnectionState == ConnectionState.WAITING_HEADER)
                {
                    HandleState_WaitingHeader(saea, tcpOffset, size);
                }
                else if (m_ConnectionState == ConnectionState.COMPLEMENT_SENT)
                {
                    HandleState_ComplementSent(saea, tcpOffset, size);
                }
                else if (m_ConnectionState == ConnectionState.FIN_SENT)
                {
                    HandleState_FinSent(saea, tcpOffset, size);
                }
                else
                {
        ...
        public void SendSegment(SocketAsyncEventArgs saea, int tcpOffset, int size)
        {
            ...
            ushort checksum = Algorithms.ChecksumTcpV4(m_PseudoHeader, segment, tcpOffset, size);
        ...
        void HandleState_WaitingSyn(SocketAsyncEventArgs saea, int tcpOffset)
        {
            byte[] segment = saea.Buffer;
            ...
            m_PseudoHeader = TcpPseudoHeader.GetPHv4(m_HostIEP.Address);
        ...
        void HandleState_WaitingHeader(SocketAsyncEventArgs saea, int tcpOffset, int size)
        {
            byte[] segment = saea.Buffer;
            int FLAG_BITS = segment[tcpOffset + 13];
            ...
            if ((FLAG_BITS & RST_SYN_FIN) != 0)
            {
                ...
                else if ((FLAG_BITS & FIN) != 0)
                {
                    ...
                    if (received_seq_num == m_next_host_sequence_number)
                    {
                        ...
                        int dataLength = size - tcp_header_size;
        ...
        void HandleState_ComplementSent(SocketAsyncEventArgs saea, int tcpOffset, int size)
        {
            byte[] segment = saea.Buffer;
            int FLAG_BITS = segment[tcpOffset + 13];
        ...
        void HandleState_FinSent(SocketAsyncEventArgs saea, int tcpOffset, int size)
        {
            byte[] segment = saea.Buffer;
            int FLAG_BITS = segment[tcpOffset + 13];
        ...
        void BuildSynAckHeader(SocketAsyncEventArgs saea)
        {
            ...
            UInt16 checksum = Algorithms.ChecksumTcpV4(m_PseudoHeader, segment, offset, header_size);
        ...
        void BuildAckHeader(SocketAsyncEventArgs saea)
        {
            ...
            UInt16 checksum = Algorithms.ChecksumTcpV4(m_PseudoHeader, segment, offset, 20);
            Buffer.BlockCopy(ByteConverter.GetBytes(checksum), 0, segment, offset + 16, 2);
        ...
        void BuildPshAckHeader(SocketAsyncEventArgs saea, int dataSize)
        {
            ...
            UInt16 checksum = Algorithms.ChecksumTcpV4(m_PseudoHeader, segment, offset, 20 + dataSize);
            Buffer.BlockCopy(ByteConverter.GetBytes(checksum), 0, segment, offset + 16, 2);
        ...
        void BuildRstHeader(SocketAsyncEventArgs saea, int tcpOffset)
        {
            byte[] segment = saea.Buffer;
            int offset = saea.Offset;

            UInt32 received_seq_num = ByteConverter.ToUInt32(segment, tcpOffset + 4);
