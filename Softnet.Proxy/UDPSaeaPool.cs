...
{
    static class UDPSaeaPool
    {
        public static int POOL_SIZE = 10000;
        public static int PKT_SIZE = 4096;
        ...
        public static void Init()
        {
            ...
            UnmanagedBuffer = new byte[POOL_SIZE * PKT_SIZE];

            for (int i = 0; i < POOL_SIZE; i++)
            {
                SocketAsyncEventArgs Saea = new SocketAsyncEventArgs();
                int offset = i * PKT_SIZE;
                Saea.UserToken = offset;
                Saea.SetBuffer(UnmanagedBuffer, offset, PKT_SIZE);
                Pool.Enqueue(Saea);
            }
        }
        ...
        {
            saea.SetBuffer((int)saea.UserToken, PKT_SIZE);
            lock (Pool)
            {
                Pool.Enqueue(saea);
            }
