...
{
    class RawUdpBindingV6
    {
        static Socket rawSocket = null;

        public static void Init(IPAddress localIP)
        {
            rawSocket = new Socket(AddressFamily.InterNetworkV6, SocketType.Raw, ProtocolType.Udp);
            rawSocket.Bind(new IPEndPoint(localIP, 0));

            rawSocket.ReceiveBufferSize = 1048576 * 100;
            rawSocket.SendBufferSize = 1048576 * 100;

            rawSocket.SetSocketOption(SocketOptionLevel.IPv6, SocketOptionName.HeaderIncluded, false);
        ...
        public static void Start()
        {
            ...
            saea.RemoteEndPoint = new IPEndPoint(IPAddress.IPv6Any, 0);
        ...
        static void InputCompleted(object sender, SocketAsyncEventArgs saea)
        {
            try
            {
                ...
                {
                    int destPort = ByteConverter.ToUInt16(saea.Buffer, saea.Offset + 2);
                    if (destPort == Constants.UdpRzvPort)
                    {
                        saea.Completed -= InputCompleted;
                        if (UdpDispatcher.HandlePacketV6(saea))
                        {
                            SocketAsyncEventArgs saea2 = UDPSaeaPool.Get();
                            if (saea2 != null)
                            {
                                saea2.RemoteEndPoint = new IPEndPoint(IPAddress.IPv6Any, 0);
