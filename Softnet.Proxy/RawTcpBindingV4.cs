...
{
    static class RawTcpBindingV4
    {
        static Socket rawSocket = null;

        public static void Init(IPAddress localIP)
        {
            rawSocket = new Socket(AddressFamily.InterNetwork, SocketType.Raw, ProtocolType.Tcp);
            ...
            try
            {
                if (saea.SocketError == SocketError.Success)
                {
                    ...
                    if (protocol == 6)
                    {
                        ...
                        {
                            saea.Completed -= InputCompleted;
                            int tcpSize = saea.BytesTransferred - ip_header_length;

                            TcpDispatcher.HandleSegmentV4(saea, next_header_offset, tcpSize, destPort);
                        ...
                        {
                            int FLAG_BITS = saea.Buffer[next_header_offset + 13];
                            if ((FLAG_BITS & SYN) != 0)
                            {
                                TcpDispatcher.StoreTCPv4Options(saea, next_header_offset);
                            }
                        }
                    }
                }
